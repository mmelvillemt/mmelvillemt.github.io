...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    Node *n = new Node(node->datum,copy_nodes_impl(node->left),
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if(node->left != nullptr 
      && less(node->datum, min_element_impl(node->left)->datum)) { 
      return false; 
    }
    if(node->right != nullptr && 
      less(max_element_impl(node->right)->datum, node->datum)) { 
      return false; 
    }
    // checks left node is > parent
    if((node->left == nullptr || less(node->left->datum , node->datum)) 
      && (node->right == nullptr || less(node->datum , node->right->datum))
